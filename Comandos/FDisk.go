...
func ValidarDatosFDISK(tokens []string) {
	...
	for i := 0; i < len(tokens); i++ {
		...
		if Comparar(tk[0], "size") {
			...
			driveletter = strings.ReplaceAll(tk[1], "\"", "")
	...
	if size == "" || driveletter == "" || name == "" {
		fmt.Println("El comando FDISK necesita parametros obligatorios")
		return
	} else {
		fmt.Println(driveletter, "este importa")
		fmt.Println(val_path, "este importa2")
		fmt.Println(unit, tipo, fit)
		generarParticion(size, unit, val_path, tipo, fit, name)
	}
}
...
func generarParticion(s string, u string, p string, t string, f string, n string) {
	...
	if i <= 0 {
		fmt.Println("Size debe ser mayor que 0")
		return
	}
	if Comparar(u, "b") || Comparar(u, "k") || Comparar(u, "m") {
		...
		fmt.Println("Unit no contiene los valores esperados.")
		return
	}
	if !(Comparar(t, "p") || Comparar(t, "e") || Comparar(t, "l")) {
		fmt.Println("Type no contiene los valores esperados.")
		return
	}
	if !(Comparar(f, "bf") || Comparar(f, "ff") || Comparar(f, "wf")) {
		fmt.Println("Fit no contiene los valores esperados.")
		return
	}
	mbr := leerDisco(p)
	...
	for j := 0; j < len(particiones); j++ {
		...
		if usado == 4 && !Comparar(t, "l") {
			fmt.Println("Limite de particiones alcanzado")
			return
		} else if ext == 1 && Comparar(t, "e") {
			fmt.Println("Solo se puede crear una partición extendida")
			return
		}
		c++
	}
	if ext == 0 && Comparar(t, "l") {
		fmt.Println("Aún no se han creado particiones extendidas, no se puede agregar una lógica.")
		return
	}
	if usado != 0 {
	...
	if regresa != nil {
		fmt.Println("El nombre: " + n + ", ya está en uso.")
		return
	}
	...
	if Comparar(t, "E") {
		...
		fmt.Println("Partición Extendida: " + n + ", creada correctamente.")
		return
	}
	file.Close()
	fmt.Println("Partición Primaria: " + n + ", creada correctamente.")
}
	...
	for {
		...
		if (tmp.Part_size == 0 && tmp.Part_next == -1) || (tmp.Part_size == 0 && tmp.Part_next == 0) {
			...
			if (ep.Part_size - size) <= logic.Part_size {
				fmt.Println("No queda más espacio para crear más particiones lógicas")
				return
			}
			...
			fmt.Println("Partición Lógica: " + nombre + ", creada correctamente.")
			file.Close()
			return
		}
		file, err = os.Open(strings.ReplaceAll(path, "\"", ""))
...
func ajustar(mbr Structs.MBR, p Structs.Particion, t []Transition, ps []Structs.Particion, u int) *Structs.MBR {
	if u == 0 {
		...
		if usar.before >= int(p.Part_size) || usar.after >= int(p.Part_size) {
			...
			fmt.Println("No hay espacio suficiente.")
			return nil
		}
	}
}
