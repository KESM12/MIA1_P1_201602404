...
func ValidarDatosLOGIN(context []string) bool {
	...
	if id == "" || user == "" || pass == "" {
		fmt.Println("Se necesitan parámetros obligatorios para el comando LOGIN.")
		return false
	}
...
func sesionActiva(u string, p string, id string) bool {
	//var path string
	letter := string(id[0])
	pat := fmt.Sprintf("/home/taro/go/src/MIA1_P1_201602404/MIA/P1/%s.dsk", letter)
	partition := GetMount("LOGIN", id, &pat)
	if string(partition.Part_status) == "0" {
		fmt.Println("No se encontró la partición montada con el id: " + id)
		return false
	}
	//file, err := os.OpenFile(strings.ReplaceAll(path, "\"", ""), os.O_WRONLY, os.ModeAppend)
	file, err := os.Open(strings.ReplaceAll(pat, "\"", ""))
	if err != nil {
	...
	for i := 0; i < len(vctr)-1; i++ {
		...
		if linea[2] == 'U' || linea[2] == 'u' {
			...
			if Comparar(in[3], u) && Comparar(in[4], p) && in[0] != "0" {
				...
				if !existe {
					fmt.Println("No se encontró el grupo \"" + in[2] + "\".")
					return false
				}
	...
	fmt.Println("No se encontró el usuario " + u)
	return false
}
